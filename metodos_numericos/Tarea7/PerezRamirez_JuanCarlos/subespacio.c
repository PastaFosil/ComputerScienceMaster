...
double **subspace_iteration(double **A, double **lambda, int n, int k, double shift){
    ...
    double **A_copy = genMatriz_double(n,n);
    ...
    for (int l=0;l<N;l++){
        ...
        printf("\nphiT\n");
        print_matrix(phiT,k,n);
        T = MxM(phiT,k,n, Q,n,k); // calculo de T
        printf("\nT\n");
        print_matrix(T,k,k);
        printf("\nl\n");
        Z = jacobi_eigen_method(T,k,5); // aplica Jacobi a T
        for (int i=0;i<k;i++){ // actualizacion de vector para evaluar convergencia
            temp[i] = T[i][i] - (*lambda)[i];
            (*lambda)[i] = T[i][i];
        }
        if (norm(temp,k)<TOL){
            free_matrix(phiT);
